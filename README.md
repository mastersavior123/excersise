# WOD Compiler — Phase 0: 지식베이스 데이터 파운데이션

나이·직업·신체정보·5대 리프트(스쿼트/데드리프트/숄더프레스/클린/스내치) 1RM을 입력받아
월간 크로스핏 프로그램을 자동 생성하는 앱의 첫 단계. 이 단계의 범위는 **UI나 생성 엔진이 아니라
`crossfit_programming_knowledge_base.xlsx`(13개 시트)를 정규화된 PostgreSQL 스키마로 옮기고,
데이터 자체의 빈틈을 측정하는 것**이다.
...
prisma/
  schema.prisma           # 지식베이스 10개 테이블 (사용자 도메인은 Phase 1에서 추가)
scripts/
  convert_xlsx_to_json.py # xlsx -> knowledge_base.json 변환 (xlsx 원본이 바뀌면 재실행)
  lib/                    # knowledge_base.json 로더, 파싱 유틸, prisma 클라이언트
...
```bash
npm install
cp .env.example .env   # DATABASE_URL을 로컬 PostgreSQL에 맞게 수정

npx prisma migrate dev --name phase0_knowledge_base
npm run db:seed
npm run db:validate
```

`db:seed`는 매번 지식베이스 테이블을 비우고 다시 채우는 멱등 스크립트다. 끝나면
`data/seed-report.json`에 실행 요약과 미해결 항목을 남긴다.
...
## 아직 남은 미확정 사항 (Phase 1 이전 결정 필요)

- **주차별 %1RM 테이블**: 지난 엔진 설계 문서의 `LOAD_TABLE_BY_FAMILY`는 여전히 제품 잠정값이며
  이번 단계에서 다루지 않았다.
- **레벨 스코어링 가중치**: `user_level_assessment` 관련 테이블/로직은 Phase 1 범위.
...
| `npm run db:seed` | knowledge_base.json → DB 적재 (전체 초기화 후 재적재) |
