  ...
  weeks: {
    ...
    include: {
      ...
      days: {
        ...
        include: {
          log: true,
          blocks: { orderBy: { orderIndex: "asc" }, include: { exercise: true, result: true } },
        },
      },
    },
  },
    ...
    for (const day of week.days) {
      ...
      for (const block of day.blocks) {
        ...
        rows.push([
          ...
          dateKey(day.date),
          day.dayType,
          day.conditioningText ? `${day.conditioningText}${day.conditioningLowIntensity ? " (저강도 전환)" : ""}` : "",
          block.slot,
  ...
  return {
    ...
    weeks: program.weeks.map((week) => ({
      ...
      days: week.days.map((day) => ({
        ...
        conditioningLowIntensity: day.conditioningLowIntensity,
        log: day.log
          ? {
        ...
        blocks: day.blocks.map((block) => ({
          ...
          result: block.result?.outcome ?? null,
        })),
      })),
    })),
